   ...
   case ESC_ARMED:
    {
      if (pHandle->Ton_value >= pESC_params->Ton_min)
      {
        ...
        /* This command sets what will be the first speed ramp after the 
      	...
      	if (pHandle->Ton_value >= pESC_params->Ton_min +20000){
    ...
    {  
      if( pHandle->Ton_value < pESC_params->Ton_min)
      {
        ...
        if(pHandle->turnoff_delay <= 0)
        {
          ...
          //MCI_ExecSpeedRamp( pMCI[pESC_params->motor], (pESC_params->speed_min_valueRPM/6), 20 );
      ...
      else 
      {
        ...
        {
        	if (pHandle->Ton_value >= pESC_params->Ton_min +20000){
        		new_speed = ((pHandle->Ton_value-pESC_params->Ton_min - (pESC_params->Ton_max-pESC_params->Ton_min)/2) * (pESC_params->speed_max_valueRPM - pESC_params->speed_min_valueRPM) / pESC_params->delta_Ton_max) + pESC_params->speed_min_valueRPM;
        	}
        	else{
